...
and a base64-encoded string of the concatenated username and password (separated by a ':') for a google account with write access to the spreadsheeta, 
will create a new worksheet in the spreadsheet and enter the data into it. Optionally, the name of the worksheet where data will be written can be 
specified, by default a new worksheet on the form [date]_[flowcell_id] is created. Alternatively, one worksheet can be created for each project 
included in the run.
...
def main(run_name, gdocs_spreadsheet, gdocs_credentials, archive_dir, analysis_dir, gdocs_worksheet, append, split_on_project):
    ...
    fp = os.path.join(archive_dir, run_name, "run_info.yaml")
